...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ adminEmail, onNavigateTab }) => {
  ...
  const handleExportDivisionsExcel = () => {
    if (!stats || !stats.divisions_summary || stats.divisions_summary.length === 0) {
      alert('Belum ada data rekapitulasi bagian untuk diekspor.');
      return;
    }
    const exportData = stats.divisions_summary.map((div, idx) => ({
      'No': idx + 1,
      'ID Bagian': div.bagian_id,
      'Nama Bagian / Divisi': div.nama_bagian,
      'Total Anggota': div.total_anggota,
      'Kuota Kursi (Rasio 10:1)': `${div.kuota_perwakilan} Kursi`,
      'Sudah Memilih (Suara Masuk)': div.sudah_memilih,
      'Belum Memilih': div.belum_memilih,
      'Partisipasi Pemilih (%)': `${div.partisipasi_persen}%`
    }));

    exportToExcel({
      filename: `Rekapitulasi_Partisipasi_Kuota_Bagian_${new Date().getFullYear()}.xlsx`,
      sheetName: 'Rekapitulasi Partisipasi',
      data: exportData
    });
  };
  ...
  const handleExportDivisionsPdf = () => {
    if (!stats || !stats.divisions_summary || stats.divisions_summary.length === 0) {
      alert('Belum ada data rekapitulasi bagian untuk dicetak.');
      return;
    }
    const printWindow = window.open('', '_blank');
    if (!printWindow) return;

    const rowsHtml = stats.divisions_summary.map((div, idx) => `
      <tr>
        <td style="text-align: center; font-family: monospace;">${idx + 1}</td>
        <td><strong>${div.nama_bagian}</strong> <span style="color:#6b7280; font-size:10px;">(${div.bagian_id})</span></td>
        <td style="text-align: center; font-family: monospace;">${div.total_anggota} Orang</td>
        <td style="text-align: center; font-family: monospace;"><strong>${div.kuota_perwakilan} Kursi</strong></td>
        <td style="text-align: center; font-family: monospace; color:#047857; font-weight:bold;">${div.sudah_memilih} Orang</td>
        <td style="text-align: center; font-family: monospace; color:#4b5563;">${div.belum_memilih} Orang</td>
        <td style="text-align: right; font-family: monospace; font-weight:bold; color:#1e3a8a;">${div.partisipasi_persen}%</td>
      </tr>
    `).join('');

    const totalMembersSum = stats.divisions_summary.reduce((s, d) => s + d.total_anggota, 0);
    const totalQuotaSum = stats.divisions_summary.reduce((s, d) => s + d.kuota_perwakilan, 0);
    const totalVotedSum = stats.divisions_summary.reduce((s, d) => s + d.sudah_memilih, 0);
    const totalUnvotedSum = stats.divisions_summary.reduce((s, d) => s + d.belum_memilih, 0);
    const overallPercent = totalMembersSum > 0 ? Math.round((totalVotedSum / totalMembersSum) * 100) : 0;
    ...
    printWindow.document.write(`
      ...
      <head>
        <meta charset="UTF-8">
        <title>Rekapitulasi Partisipasi & Alokasi Kuota - KOPSYAH YKK AP</title>
        <style>
          @page { size: A4 portrait; margin: 15mm; }
          body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; padding: 20px; color: #111827; background: #fff; }
          ...
          .header h2 { font-size: 16px; color: #1e3a8a; margin: 4px 0; font-weight: 800; }
          .header p { font-size: 11px; color: #4b5563; margin: 0; }
          .title { text-align: center; margin-bottom: 16px; }
          .title h3 { font-size: 14px; margin: 0; text-transform: uppercase; text-decoration: underline; font-weight: 800; }
          .summary-box { display: flex; justify-content: space-around; background: #f9fafb; border: 1px solid #d1d5db; padding: 10px; border-radius: 8px; margin-bottom: 16px; font-size: 11px; text-align: center; }
          table { width: 100%; border-collapse: collapse; font-size: 11px; margin-top: 8px; }
          th, td { border: 1px solid #9ca3af; padding: 7px 10px; }
          th { background: #f3f4f6; text-align: left; font-weight: bold; color: #1f2937; }
          tfoot tr td { font-weight: bold; background: #f9fafb; }
          .action-bar { display: flex; justify-content: flex-end; gap: 8px; margin-bottom: 16px; }
          .btn { padding: 6px 12px; font-size: 11px; font-weight: bold; cursor: pointer; border-radius: 4px; border: 1px solid #ccc; background: #fff; }
          .btn-primary { background: #1e3a8a; color: #fff; border: none; }
          @media print { .action-bar { display: none !important; } body { padding: 0 !important; } }
        </style>
      </head>
      <body>
        ...
        <div class="header">
          <h1>KOPERASI KARYAWAN SYARIAH (KOPSYAH)</h1>
          <h2>KOPSYAH YKK AP INDONESIA</h2>
          <p>Laporan Rekapitulasi Partisipasi & Alokasi Kuota Perwakilan per Bagian</p>
        </div>
        <div class="title">
          <h3>REKAPITULASI PARTISIPASI & ALOKASI KUOTA PER BAGIAN</h3>
        </div>
        <div class="summary-box">
          <div><strong>Total Anggota:</strong> ${totalMembersSum} Orang</div>
          <div><strong>Total Kuota Kursi:</strong> ${totalQuotaSum} Kursi</div>
          <div><strong>Sudah Memilih:</strong> ${totalVotedSum} Orang</div>
          <div><strong>Partisipasi Nasional:</strong> ${overallPercent}%</div>
        </div>
        <table>
          <thead>
            <tr>
              ...
              <th style="text-align: center;">Belum Memilih</th>
          ...
          <tfoot>
            <tr>
              ...
              <td style="text-align: center;">${totalMembersSum} Orang</td>
              <td style="text-align: center;">${totalQuotaSum} Kursi</td>
              <td style="text-align: center;">${totalVotedSum} Orang</td>
              <td style="text-align: center;">${totalUnvotedSum} Orang</td>
              <td style="text-align: right;">${overallPercent}%</td>
            </tr>
          </tfoot>
        </table>
        <script>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-2xs overflow-hidden">
        <div className="p-5 sm:px-6 border-b border-gray-100 flex items-center justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            <button
              type="button"
              onClick={handleExportDivisionsExcel}
              className="text-xs font-bold uppercase tracking-wider text-emerald-800 hover:text-emerald-900 flex items-center gap-1.5 px-2.5 py-1 rounded-lg border border-emerald-300 bg-emerald-50 hover:bg-emerald-100 transition-colors cursor-pointer shadow-2xs"
              title="Ekspor Rekapitulasi per Bagian ke Excel"
            >
            ...
            <button
              type="button"
              onClick={handleExportDivisionsPdf}
              className="text-xs font-bold uppercase tracking-wider text-blue-900 hover:text-blue-950 flex items-center gap-1.5 px-2.5 py-1 rounded-lg border border-blue-300 bg-blue-50 hover:bg-blue-100 transition-colors cursor-pointer shadow-2xs"
              title="Cetak Laporan Rekapitulasi / Simpan PDF"
            >

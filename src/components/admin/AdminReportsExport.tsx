...
export const AdminReportsExport: React.FC = () => {
  ...
  const totalMembersFromResults = results.reduce((sum, r) => sum + (r.total_anggota || 0), 0);
  const totalMembers = Math.max(totalMembersFromMembers, totalMembersFromResults);

  const totalVotesCastFromResults = results.reduce((sum, r) => sum + (r.total_suara_masuk || 0), 0);
  const totalVotesCastFromMembers = members.filter(m => m.status_memilih === 'SUDAH_MEMILIH').length;
  const totalVotesCast = Math.max(totalVotesCastFromResults, totalVotesCastFromMembers);
  ...
  const handleExportFullResultsExcel = () => {
    ...
    results.forEach(d => {
      const voted = d.total_suara_masuk || 0;
      const unvoted = Math.max(0, (d.total_anggota || 0) - voted);
      ...
      d.candidates.forEach(c => {
        ...
        exportData.push({
          ...
          'Anggota Memilih': voted,
          'Anggota Tidak Memilih': unvoted,
          ...
          <div className="border border-gray-300 rounded-xl overflow-hidden">
            <table className="w-full text-left text-xs border-collapse">
              <thead className="bg-gray-100 font-bold text-gray-700 uppercase tracking-wider border-b border-gray-300">
                <tr>
                  <th className="py-2.5 px-2.5 border-r border-gray-300 w-10 text-center">No</th>
                  <th className="py-2.5 px-3 border-r border-gray-300">Bagian / Divisi</th>
                  <th className="py-2.5 px-2.5 border-r border-gray-300 text-center w-24">Kuota Kursi</th>
                  <th className="py-2.5 px-2.5 border-r border-gray-300 text-center w-28">Anggota Memilih</th>
                  <th className="py-2.5 px-2.5 border-r border-gray-300 text-center w-32">Anggota Tidak Memilih</th>
              ...
              <tbody className="divide-y divide-gray-200 text-gray-800">
                {results.map((divRes, idx) => {
                  ...
                  const voted = divRes.total_suara_masuk || 0;
                  const unvoted = Math.max(0, (divRes.total_anggota || 0) - voted);

                  return (
                    <tr key={divRes.bagian_id}>
                      <td className="py-2.5 px-2.5 text-center font-mono border-r border-gray-200">
                        {idx + 1}
                      </td>
                      ...
                      <td className="py-2.5 px-2.5 text-center font-mono font-bold text-emerald-700 border-r border-gray-200">
                        {voted} Orang
                      </td>
                      <td className="py-2.5 px-2.5 text-center font-mono font-bold text-gray-500 border-r border-gray-200">
                        {unvoted} Orang
                      </td>
